...
/* This gets everything on a single line. 
By default, the direction is in a row and in standard order. 
*/
...
This takes the elements and places them in a single row or a single column. 
Ordering is either in source order or the reverse of the source order. 
Flex-direction defines our main axis. */

3. 
ul { 
    display: flex; 
    flex-direction: as before, 
    flex-wrap: ***;
}
...
Justify content determines the distribution of the flex-items within the flex-container on the main axis — in other words, 
how should space be allocated relative to the width of each item?
If flex-direction is row, then horizontal is the main axis. When flex-direction is column, 
then column is the main axis. */
...
/* This artificially gives our row some height. In the next step, 
we’ll try some cross axis alignment. Since we’re working with a row, 
...
This aligns our items on the cross axis. Since we’re working with a row currently, 
this is aligning elements in vertical space.  */
